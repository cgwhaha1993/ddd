var changeCarList = (function(){
    ...
    function logoFixed(len){
        var topFixed = document.getElementsByClassName('g_top_m')[0];
        document.onscroll = function(){
            if(window.scrollY>len){
                topFixed.className = 'g_top_m top_fixed';
            }else{
                topFixed.className = 'g_top_m';
            }
        }
    }
    logoFixed(120);
    //搜索框点击效果
    // var searchText = document.getElementsByClassName('search_text')[0];
    ...
    //     check_out_span.href = 'good_car.html';
    ...
    function changeCarList(){
        ...
        carlist_arr.forEach(function(item){
         ...
         car_total_price.innerHTML = 0;
        function createCarBox(arr){
            if(g_car_items.children[1]){g_car_items.removeChild(g_car_items.children[1]);}
            if(!arr.length){
                return;
            }
            var car_items_box = document.createElement('div');
    ...
    var nav_menu_timer;
    nav_menu.onmouseenter = function(){
    ...
    nav_menu.onmouseleave = function(){
        nav_menu_timer = setTimeout(function(){
            nav_menu_one.style.display = 'none';
        },600);
    }
